...
class MainDetailViewModel(savedStateHandle: SavedStateHandle) : ViewModel() {

    val sigren: String? = savedStateHandle[NavArgs.ItemId.key]
    ...
    val favorites = getFavoritesRoutes(savedStateHandle[NavArgs.ItemId.key] ?: "")
    ...
    fun saveFavorite(feature: GeoJsonServerResult.Feature) {
        viewModelScope.launch {
            sigren?.let { FirebaseService.saveFavorite(it, feature) }
        }
    }
    fun removeFavorite(feature: GeoJsonServerResult.Feature) {
        viewModelScope.launch {
            sigren?.let { FirebaseService.removeFavorite(it, feature) }
        }
    }
    ...
    init {
        viewModelScope.launch {
            sigren?.let {
                ...
                state = UiState(data = result.data, loading = false)
